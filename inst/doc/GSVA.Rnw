...
edgeRenderInfo(g2plot) <- list(lwd=2)
...
$n=$\Sexpr{dim(leukemiaKEGG_es)[2]}, we will employ all possible $q$ orders,
as in \citep{Castelo_qpgraph_2009}.

<<>>=
cache(avgnrr <- qpAvgNrr(leukemiaKEGG_es, qOrders=1:(dim(leukemiaKEGG_es)[2]-3),
                         verbose=FALSE),
                         dir=cacheDir, prefix=cachePrefix)
@
...
qpclq <- qpClique(avgnrr, N=dim(leukemiaKEGG_es)[2], breaks=10)
@
<<avgnrrcliquenr, echo=FALSE, results=hide>>=
dev.off()
@
...
The resulting network is shown in Figure~\ref{fig:LeukemiaNet}. Among the selected cross-talk
associations, the one between "Mismatch repair" (KEGG ID KO03430) and "One carbon pool by folate"
(KEGG ID HSA00670) is quite relevant in the context of these data. Folate is an essential
nutrient for basic processes as the synthesis, repair and methylation of DNA and which has been
shown to play a role in prevention of many diseases including neural tube defects,
cardiovascular disease, and cancer where genetic variation in folate metabolism has been reported
to be associated to childhood leukemia \citep{pmid11747917}. The genes that form this pathway,
