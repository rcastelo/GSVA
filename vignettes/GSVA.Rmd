...
The main argument to the `gsva()` function is a parameter object
collecting and validating the input to the GSVA algorithm that it
causes `gsva()` to apply.  The first argument to the `gsvaParam()`
function constructing this parameter object is the gene expression
data matrix and the second the collection of gene sets. The `gsva()`
function can take the input expression data and gene sets using
different specialized containers that facilitate the access and
manipulation of molecular and phenotype data, as well as their
associated metadata. Another advanced features include the use of
on-disk and parallel backends to enable, respectively, using GSVA on
large molecular data sets and speed up computing time. You will find
information on these features in this vignette.
...
objects corresponding to the methods listed above and their particular
parameter lists but all of them require at a minimum the following two input
arguments:
...
The method employed by the `gsva()` function is determined by the
class of the parameter object that it receives as an input.  An
object constructed using `gsvaParam()` runs the method described by
@haenzelmann_castelo_guinney_2013 but this can be changed using
`plageParam()`, `zscoreParam()`, or `ssgseaParam()`, corresponding
to the methods briefly described in the introduction.
...
* `tau`: Exponent defining the weight of the tail in the random
  walk. By default `tau=1`. When `method="ssgsea"`, an analogous
  parameter is used but called `alpha` and its default value becomes
  then `alpha=0.25` to match the methodology described in
...
```{r, results="hide"}
esmicro <- gsva(gsvaParam(huangArrayRMAnoBatchCommon_eset, canonicalC2BroadSets, minSize=5, maxSize=500))
esrnaseq <- gsva(gsvaParam(pickrellCountsArgonneCQNcommon_eset, canonicalC2BroadSets, minSize=5, maxSize=500, kcdf="Poisson"))
```
...
```{r, results="hide"}
gbm_es <- gsva(gsvaParam(gbm_eset, brainTxDbSets, maxDiff=FALSE))
```
...
```{r, results="hide"}
leukemia_es <- gsva(gsvaParam(leukemia_eset, c2BroadSets, minSize=10, maxSize=500))
```
